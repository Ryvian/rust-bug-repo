...
impl LeaderClient {
    ...
    pub async fn reconnect(&self) -> Result<()> {
        ...
        match try_connect_leader(
            Arc::clone(&self.inner.rl().env),
            Arc::clone(&self.inner.rl().security_mgr),
            self.inner.rl().members.clone()) {
            Ok((cli, mbrs)) => {
                let mut inner = self.inner.wl();
                inner.client_stub = cli;
                inner.members = mbrs;
            }
            Err(e) => {
    ...
    F: Fn(&PdClientStub) -> GrpcResult<R>,
{
    ...
    for _ in 0..retry {
        ...
        match func(&client.inner.rl().client_stub).map_err(Error::Grpc) {
            Ok(r) => {
                return Ok(r);
            }
            Err(e) => {
...
pub fn validate_endpoints(
    ...
    match members {
        Some(members) => {
            let (client, members) =
                try_connect_leader(Arc::clone(&env), security_mgr, members)?;
...
pub fn try_connect_leader(
        ...
        .chain(&[previous_leader.clone()])
    {
        for ep in m.get_client_urls() {
            match block_on(connect(Arc::clone(&env), &security_mgr, ep.as_str())) {
                Ok((_, r)) => {
    ...
    // Then try to connect the PD cluster leader.
    {
        if let Some(resp) = resp {
            ...
            for ep in leader.get_client_urls() {
                if let Ok((client, _)) = block_on(connect(Arc::clone(&env), &security_mgr, ep.as_str())) {
